...
final class SpeechAnalyzerEngine: SpeechRecognitionEngine {
  ...
  func prepare(language: RecognitionLanguageOption) async throws {
    ...
    let sourceLocale = RecognitionLanguageResolver.locale(for: language).identifier
    NSLog(
      "Speech engine preparing: option=\(language.rawValue), source=\(sourceLocale), resolved=\(locale.identifier)"
    )
